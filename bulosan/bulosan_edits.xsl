...
<!-- You have to add the namespace declaration: "xmlns:tei" as an attribute in
    the root: this says all expression with "tei" prefix are in the TEI namespace. Then you
have to add all the prefixes below -->
    ...
    <xsl:template match="/">
        <html>
            <head>
                ...
                <!-- Ideally, you would do a lot of CSS in a <style> element here in the <head>, 
                    rather than add all the CSS inline -->
            </head>
            <body style="font-family: 'Courier New', Monospace; padding: 20px; max-width: 950px; margin: 0 auto; line-height: 1.1; background-color: #F2F1E6;">
                ...
                <!-- This is the content that was in your <teiHeader> template. But you don't want it there.
                    You want it here, as part of the HTML frame -->
                ...
                <!-- Then the apply-templates instruction here tells the processor to 
                    continue to apply templates, and output the rest of the content, 
                    in accordance with all the templates below -->
                <xsl:apply-templates/>
            </body>
        </html>
    </xsl:template>
    
    <!-- You want this to be an empty element, which simply suppresses all the content
        in the <teiHeader>. You don't want to add content to this. I moved all the content you
        had here above, into the <html> frame. -->
